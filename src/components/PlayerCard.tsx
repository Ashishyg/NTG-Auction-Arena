...
export function PlayerCard({
  ...
  if (!player) {
    return (
      <div className="glass grid min-h-[18rem] place-items-center p-10 text-center">
        <div>
          <div className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl border border-white/[0.07] bg-white/[0.03] text-2xl">
            🎯
          </div>
          <p className="text-[10px] uppercase tracking-[0.28em] text-white/40">No player on the block</p>
    ...
    <div className="glass-strong overflow-hidden">
      <div className="p-6 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[10px] font-medium uppercase tracking-[0.28em] text-white/40">{game}</p>
            <h2 className="mt-1 font-display text-4xl leading-tight text-gradient-brand">{player.name ?? "Unknown"}</h2>
          </div>
          <div className="text-right">
            <p className="text-[10px] uppercase tracking-[0.2em] text-white/40">{live ? "Current bid" : "Floor"}</p>
            <p className="font-display text-5xl leading-none text-gold tabular-nums">{price ?? 0}</p>
          </div>
        </div>
      ...
      <div className="border-t border-white/[0.07] bg-white/[0.015] px-6 py-3.5 sm:px-7">
        {highestBidderName ? (
